...
# @Last Modified time: 2024-04-11 09:19:17
...
# def parse_xml_to_gbpluginmethods(xml_data: str, work_path: str, out_path: str) -> GBPluginMethods:
#     root = etree.fromstring(xml_data)
#     participants = {p.get('id'): p.get('name') for p in root.xpath('//talkbank:Participants/talkbank:participant', namespaces=root.nsmap)}

#     utterances = {}
#     for u in root.xpath('//talkbank:u', namespaces=root.nsmap):
#         speaker = participants.get(u.get('who'), u.get('who'))
#         start = float(u.xpath('talkbank:media/@start', namespaces=root.nsmap)[0])
#         end = float(u.xpath('talkbank:media/@end', namespaces=root.nsmap)[0])
#         text = ' '.join(w.text for w in u.xpath('talkbank:w', namespaces=root.nsmap))
#         file_name = "merged"  # This should be derived based on your actual file structure
#         if file_name not in utterances:
#             utterances[file_name] = []
#         utterances[file_name].append({
#             "speaker": speaker,
#             "start": start,
#             "end": end,
#             "text": text
...
#     data_files = ["path/to/audio1", "path/to/audio2"]  # Update with actual paths
#     merged_media = "path/to/merged_audio"
